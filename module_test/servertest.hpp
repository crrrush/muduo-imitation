...
// template <class T>
// class get_shared_ptr_from_this
// {
// private:
//     std::weak_ptr<T> _weak_ptr_to_this;

// public:
//     get_shared_ptr_from_this(const std::shared_ptr<T> &slef_sp) : _weak_ptr_to_this(slef_sp) {}

//     std::shared_ptr<T> shared_from_this() { return _weak_ptr_to_this.lock(); }
// };
...
#define BUFFSIZE 65536
    ...
    void handle_write()
    {
        ...
        if (-1 == n)
        {
            ...
            // LOG(DEBUG, "[will be call release_in_loop][fd:%d]", _sockfd);
...
public:
    ...
    // public:
    //     static std::unique_ptr<connection_manager> instance;
...
public:
    ...
    // static void delconn(const conn_ptr &pc) { instance->delconn_by_ptr(pc); }

    // static void delconn(connection_manager &manager, const conn_ptr &pc) { manager.delconn_by_ptr(pc); }
    // static void delconn(connection_manager *pmanager, const conn_ptr &pc) { pmanager->delconn_by_ptr(pc); }

    // 获取连接
    // conn_ptr getconn_ptr(const int &fd) { return _conns[fd]; }
    conn_ptr getconn_ptr(const int &conn_id) { return _conns[conn_id]; }

    // conn_ptr operator[](const int &fd) { return _conns[fd]; }
...
// // 管理类唯一实例 使用unique_ptr管理 new出一个实例
// std::unique_ptr<connection_manager> connection_manager::instance(new connection_manager);

// const static std::unique_ptr<connection_manager> &conn_manager = connection_manager::instance;
...
public:
    ...
    //     if (0 == _thread_num)
    //         return _main_loop;
    //     return _loops[_loop_index++ % _thread_num];
    // }

    // 获取连接管理器
    // connection_manager &which_connection_manager() { return _conn_managers[_loop_index]; }
};
